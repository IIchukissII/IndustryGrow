...
3. **Physical quantities use the standard SI sample types.** Continuous measurements are published as `uavcan.si.sample.*` types — each carries a synchronized timestamp and an SI-unit float — never as bespoke per-sensor records. In particular the M05 set maps as:
   ...
   SI base units on the wire (kelvin, joule) are deliberate: unit conversion for human display is a gateway/platform concern, not a node concern, and keeping the wire in SI removes a class of unit-mismatch bugs.
...
- Reuse-first keeps the project's owned type surface small — for M05, essentially the standard SI samples plus two tiny `safety` status types — minimizing maintenance and maximizing tool interoperability.
