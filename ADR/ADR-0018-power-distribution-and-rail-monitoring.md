...
- **ADR-0014** — refines M05-SAFETY. Power/current monitoring moves from per-load INA226s on the per-zone module to a **single** source-side INA226 on the central board (the `+12 V` sensor bus), plus a DIN kWh meter over S0 for all actuator / high-power energy; per-individual-load **and** per-section DC monitoring are dropped, not relocated. The board *is* the M05 module (carrier mounted on it; straps `0b101`; standard header contract). Safety interlocks (TMP117/reed/leak) remain on leads per decision 3. The M05 description in ADR-0014 is reconciled by an inline refinement note (rev 1).
