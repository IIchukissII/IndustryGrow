...
| `E0011` | A01-THERMAL actuator module | electrical | Module class ID `0x80` — first actuator class, so it identifies by EEPROM at I2C `0x50` and needs carrier `E0001-000100` (`O-100`). Bidirectional grow-volume temperature: four series thermoelectric modules on the `+24 V` section, current-mode buck plus H-bridge, three hardware interlocks (hot face, grow volume per ADR-0018 d10, coolant flow). Not yet laid out. Specification: `spec/A01-THERMAL-specification.md`. |
