...
|----------|-------------|------------|-------|
| `E0001` | Universal carrier | electrical | One bare design, one assembly — no real variant (ADR-0017 decision 4). Hosts the WeAct core board (`SP0005`). Two revisions in service, each with its own firmware image: `E0001-000003` identifies a module by the 3-bit strap; `E0001-000100` identifies it by the 8-bit class ID in the module's EEPROM, carries no strap, and widens Header B to 2×10 for two further analog channels (ADR-0014 rev 4 d6). Both header contracts and both pin assignments are in the specification. Specification: `spec/E0001-R-specification.md`. |
| `E0002` | M01-CLIMATE sensor module | electrical | Module-ID strap `0b001`. Air *state* at the canopy: SHT45, BME688, SCD41 — all board-mounted. Airflow moved to `E0008` (ADR-0014 rev 2). `E0002-000001` fabricated; bench bring-up 2026-08-24, all ten subjects published; node draw measured (V3, 2026-08-26). Specification: `spec/E0002-R-specification.md`. |
| `E0003` | M02-LIGHT sensor module | electrical | Module-ID strap `0b010`. Photic: AS7343 14-channel spectral + TSL2585 UV-A, separated by a TCA9543A I2C switch since both answer at 0x39 (ADR-0014 rev 6). `E0003-000001` carries this complement in both schematic and layout, and is fabricated. Specification: `spec/E0003-R-specification.md`. |
...
| `E0005` | M04-PLANT sensor module | electrical | Module-ID strap `0b100`. Plant-level: MLX90640 thermal imager, 32x24, 110° × 75° FOV population. `E0005-000001` carries this complement in both schematic and layout. Not yet fabricated. Specification: `spec/E0005-R-specification.md`. |
| `E0006` | M05-SAFETY / cabinet distribution + monitoring board | electrical | Module-ID strap `0b101`. Sense-only. Single INA226 on the `+12 V` SELV bus, TMP117, reed, leak, on-board input fuse, DIN-meter S0 input (ADR-0018). Node draw measured 0.25 W with carrier. Specification: `spec/E0006-R-specification.md`. |
...
| `E0008` | M06-VENTILATION sensor module | electrical | Module-ID strap `0b110`. Air *transport* through the cabinet: FS3000 velocity (moved here from `E0002`), two SDP8xx-class differential-pressure ranges (filter loading, envelope↔ambient), SHT45 for in-stream density compensation. Introduced by ADR-0014 rev 2; not yet laid out. Specification: `spec/E0008-R-specification.md`. |
| `E0009` | M07-AMBIENT sensor module | electrical | Module-ID strap `0b111`. Boundary conditions one step outside the growing enclosure: SHT45, BMP390-class barometric, SCD41 reference, irradiance. Two **populated-BOM variants of this one design** (ADR-0014 rev 2): *indoor* — the room around a cabinet, core sensors only; *outdoor* — the weather, adding the deployment-scale wind group (purchased anemometer + Gray-coded vane, LSM303AGR heading / installation-integrity monitor) and outdoor qualification. One PCB layout, one module class, one strap, one firmware image — but **two assembly E-numbers**, since a wind group fitted or omitted at build is a real populate variant (ADR-0017 rev 2 decision 4). `E0009` names the first; the second is assigned at design commit, not pre-reserved. Introduced by ADR-0014 rev 2; not yet laid out. Specification: `spec/E0009-R-specification.md`. |
| `E0010` | Service tool - local operator and diagnostic interface | electrical | Optional accessory, never equipment: the cabinet operates, controls and records identically without it (ADR-0030 d1). Joins the bus listen-only, executes a carried `-M-` protocol step by step, and emits the per-instance record it does not keep. Delivered realization open (`O-76`); two bench realizations run today (spec section 2.1). Emitted records identify the tool by its board serial. Firmware: `service-tool/`, one codebase, build variants `-F-h753` / `-F-h757` (ADR-0017 d16). Specification: `spec/E0010-R-specification.md`. |
| `E0011` | A01-CLIMATE actuator module | electrical | Module class ID `0x80` — first actuator class, so it identifies by EEPROM at I2C `0x50` and needs carrier `E0001-000100` (`O-100`). Acts on the grow-volume air and carries every regulated variable of that medium (ADR-0031 rev 1 d2): a main tract whose two-module thermoelectric stack sets air temperature, a humidity tract whose subcooler and reheater set humidity ratio, and an unpopulated CO₂ branch. Two DRV8262 stages on the `+24 V` section plus a module-local I²C PWM expander; four hardware interlocks (water block, grow-volume window per ADR-0018 d10, coolant flow, tract airflow). Not yet laid out. Specification: `spec/E0011-R-specification.md`. |
| `E0012` | A01-CLIMATE tempering unit | mechanical | The main-tract assembly of the A01-CLIMATE apparatus, whose class spans both E-numbers (ADR-0031 rev 1 d2): HX1 with its integral `E2` fan, the `E1` thermoelectric pair, WB1 as rejection face and clamping plate, and the sensing elements U1, U2 and RT1, in a printed housing. Carries no electronics and no class ID; every element is commanded by `E0011` over a harness. Serialized, with dimensional and clamping-force inspection in its `-QP` (ADR-0017 d10). First mechanical E-number, discipline held here as metadata (ADR-0017 d5). Not yet built. Specification: `spec/E0012-R-specification.md`. |
...
| `SP0003` | DIN-rail SELV power supply, `+12 V` output, OVP/OCP | no | Output power/current per deployment in the BOM. Phase 1 is `+12 V` only; the `+24 V` power section is `SP0010` (ADR-0018). |
...
| `SP0007` | Fan-cooled extruded heatsink, 62 × 74 mm section, 150 mm length, `Rth` 0.17 K/W with `SP0008` fitted | no | HX1 of `E0012`. The Fischer LA 6/150 carries this section; its 24 V catalogue aggregate is 0.225 K/W at 46 m³/h and its own fan does not meet `SP0008`, so a pre-fitted catalogue item does not satisfy both lines together. Spreading resistance is unmeasured and the catalogue figure misses `Rth` before spreading (`O-112`). |
| `SP0008` | Axial fan, 60 × 60 × 25 mm, 24 V DC over 18–26 V, ≥ 56 m³/h free air at ≤ 3 W, ball bearing, L10 ≥ 60 000 h at 40 °C, externally commandable duty | no | The `E2` element of `E0012` and, from 2026-09-10, `E0011`'s `E5` as well — one fan part for both air tracts. Mounted on `SP0007` in the main tract. Sized on the **ebm-papst 614 NHH**, the 24 V member of the 612/614 pair; the 612 is the 12 V member and does not meet this line. Parameters are in `spec/E0012-R-specification.md` §4; any part meeting the spec substitutes (ADR-0019 d3). A speed signal, a duty input and a go/no-go alarm exist only as custom variants; which variant is ordered decides `E0011`'s `T5` interlock and both fans' drive (`O-128`). An actuator device under d6, and the unit's one wear part, so it carries its own line rather than disappearing into the heatsink's. |
...
| `SP0010` | SELV power supply, `+24 V` output, OVP/OCP, actuator section | no | Feeds every commanded element of A01-CLIMATE, the `SP0008` fan included; no actuator element sits on the `+12 V` sensor bus (ADR-0018 d3). Output power and current per deployment in the BOM, and where a configurable chassis is used the chassis and its output module are BOM lines under this number. Startup, inhibit and output-isolation behaviour unconfirmed (`O-104`). |
| `SP0011` | Extruded aluminium finned heatsink profile, anodised: 40 mm wide, base ≥ 6 mm, fin height 20 mm, fin gap 2.0–2.2 mm, fin thickness ≤ 1.0 mm, supplied in cut lengths, no integral fan | no | HX2 (80 mm) and HX3 (40 mm) of `E0011` — the humidity tract's cold and reheat sections, one profile at two lengths. The fin gap is a range and not a minimum because the air-side conductance falls steeply above it; the duty it must meet is `T12` of `spec/E0011-R-specification.md`. The tract's only commanded air mover is `E5`, so a catalogue aggregate carrying its own fan does not satisfy this line. |
