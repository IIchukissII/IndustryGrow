/* Taken from the HAL template: FreeRTOS owns SysTick, so the HAL takes its
 * 1 kHz time base from TIM6 instead. HAL_GetTick() is unchanged for every
 * caller -- the model, the liveness thresholds and LVGL all still read it.
 */
/**
  ******************************************************************************
