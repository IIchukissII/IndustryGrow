...
| U2 | `BME680` | Temperature accuracy ±1.0 °C instead of ±0.5 °C; pressure accuracy degrades; no parallel mode; §6.4 does not use it, so the sweep is unaffected. Requires the firmware discrimination of §10 |
...
| Requirement | Value |
|-------------|-------|
| Output | Gas resistance, uncalibrated. Vector, not scalar: R(T) at each setpoint below. Trend, not a concentration. Closes O-49 |
| Setpoints | 200, 250, 320, 400 °C; 150 ms dwell each; consecutive forced-mode shots, ~760 ms per scan. Parallel mode is not commanded, so the §4.2 alternative produces the same vector |
| Scan interval | 10 s. Hotplate duty 6 % (4 × 150 ms per 10 s), against 15 % at a 1 s interval |
| Parked state | Hotplate not commanded, 4117 not published. Pressure and the secondary T/RH are unaffected — they come from a conversion on every publish tick with the gas step disabled |
| Measured response | +187 % at 200 °C, +128 % at 250 °C, +65 % at 320 °C, +38 % at 400 °C over one breath stimulus; R(200 °C)/R(400 °C) 2.20 → 4.56 |
...
| Drift | Hotplate ageing; per-device baseline moves |
...
FB1 and U5 attenuate different bands and neither is optional: FB1 alone leaves the envelope,
U5 alone leaves the 400 kHz component in its PSRR trough.
...
Rail ripple is unmeasured — no oscilloscope is available. The design is dimensioned against the
datasheet limit, not against the rail. O-44.
...
| Recorded in the design | `4.7uF`, `0805`, distributor code `C1779`. Dielectric, voltage rating and manufacturer part are not captured. O-73 |
| Local decoupling | 100 nF at U3's supply pins, in addition and not as a substitute |
| Transient check | Excursion at a 190 mA step shall stay inside 400 mV, against U5's published transient-response curve |
...
Supplying U2 at 1.8 V relocates its dissipation from U2's die to U4. T3 applies to U4 and U5.
...
| Commanded operations | Vendor `uavcan.node.ExecuteCommand` IDs, none automatic. Every one that stops U3 runs across the loop rather than inline, with nothing else addressing U3 meanwhile |
| | 1 = U3 self-test: 10 s, U3 stopped and reconfigured around it |
| | 2 = U1 condensate-recovery pulse, fired immediately after a U3 poll so it cannot overlap a measurement window |
| | 3 = write U3's temperature offset. `parameter` carries centi-degrees Celsius as decimal ASCII; a value outside 0–2000 is rejected. Stop, write, persist, reconfigure — issued once per determination, never on a schedule, because the device EEPROM is rated for 2000 cycles |
| | 4, 5 = park and arm U2's gas scan |
| | 6, 7 = stop and restart U3's periodic measurement |
...
| Health | Heartbeat health is the worse of identity and sensor state. A device is faulted when it is present and its reads fail, **or when it was present earlier in this boot and has since vanished** — every publication is gated on the device flag, so a device that disappears takes its subjects with it and must not read as healthy. A device that was never present is an absence, not a fault (O-37). CAUTION for U1, ADVISORY for U2 or U3, or when identity is not ATECC-anchored. Three consecutive failed cycles is the threshold |
...
| 4117 | Gas resistance sweep | U2 | `industryflow.greenhouse.climate.GasResistance.2.0` — setpoints and resistances index-for-index. Rate per §6.4 |
...
unscaled — ratio, mole fraction, ohm (ADR-0005 d2).
...
| V3 | §7.1, §7.4 | Node draw taken **differentially** on M05's shunt — the `+12 V` bus current with this node present, less the same bus with it removed. The difference cancels the shunt offset that dominates M05's absolute accuracy and the indicator tap upstream of it (M05 specification O-25), so it is far tighter than either reading. Bus current is M05's published mean over each publication period. The burst is the same current sampled at 200 Hz, taken as the difference between U3 running and U3 stopped (§10 commands 7 and 6): at that rate the carrier's pulse-skipping draw swings the bus about ±10 mA on its own, so stopping U3 excludes it rather than letting it be read as burst. Rail voltages at U2 VDD and U3 VDD under load, and ripple against §7.4.1's 30 mV, need instrumentation the project does not have — O-44 | Node draw and burst executed. Rail voltages and ripple not executed. O-44 |
...
| ~~O-49~~ | ~~Whether the node publishes U2's resistance vector or a reduction of it~~ — closed by §6.4 in favour of the vector, taken as consecutive forced-mode shots so both §4.2 variants produce it | — |
