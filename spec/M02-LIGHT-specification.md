...
|--------------------|--------|--------------|--------------------------|----------|
| Spectral counts, 11 bands, 405…855 nm | U1 | 0…65 535 counts per band, 16-bit | 10…90 % of full scale, held there by autorange (§6.1); zero in the dark period | Relative between bands; absolute only through §6.2 |
...
U1 is the primary photic source. U2 measures UV only and contributes nothing to the PPFD
...
available as the carrier debug console throughout boot and run (pin map note 6). U1's `INT`,
...
PPFD is a photon count over 400–700 nm. U1 delivers filtered band counts at 405…690 nm centres
...
| 365–385 nm UV-A | none of U1's bands — F1's half-maximum starts at 390 nm | U2's function, §6.4 |
...
U3 dissipates 1.5 V × 280 µA = 0.42 mW. Incident radiation dominates, and it is a property of
the installation.
...
|----|----------|--------|------|
| 4128 | Spectral counts, 11 bands + clear, with AGAIN and `t_int` | U1 | `industryflow.greenhouse.light.SpectralSample` |
...
| 4132 | UV-B irradiance | U2 | `industryflow.greenhouse.light.Irradiance` (W·m⁻²) |
| 4133 | UV-C irradiance | U2 | `industryflow.greenhouse.light.Irradiance` (W·m⁻²) |

Six subjects, not one record: a partial population must be able to omit U2's three
(ADR-0005 d8). U1's channels are one device read in one acquisition and are not separable, so
